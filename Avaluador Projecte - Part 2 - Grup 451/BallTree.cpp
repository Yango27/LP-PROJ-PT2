...
Coordinate BallTree::nodeMesProper(Coordinate targetQuery, Coordinate& Q, BallTree* ball) {
    ...
    if (ball->m_left == nullptr && ball->m_right == nullptr && !ball->m_coordenades.empty()) //si ball es una fulla
    {
    ...
    else //si no es fulla apliquem la recursivitat
    {
        if (ball->m_left != nullptr && ball->m_right != nullptr)
        {
            ...
            if (Da < Db)
            {
                ball->nodeMesProper(targetQuery, Q, ball->m_left);
                ball->nodeMesProper(targetQuery, Q, ball->m_right);
            }
            else
            {
                ball->nodeMesProper(targetQuery, Q, ball->m_right);
